...
try:
    ...
    def RobustLogger(*args, **kwargs):  # type: ignore
        return logging.getLogger("app_process_lifecycle")
...
from app_process_lifecycle._compat import is_frozen
...
def start_shutdown_process(main_pid: int | None = None):
    ...
    args = [
        sys.executable,
        "-c",
        f"from app_process_lifecycle.shutdown import shutdown_main_process; shutdown_main_process({os.getpid()})",
    ]
    if is_frozen():
...
def terminate_child_processes(
    ...
    for child in active_children:
        ...
        try:
            ...
            try:
                if os.name == "nt":
                    from app_process_lifecycle._compat import win_get_system32_dir
